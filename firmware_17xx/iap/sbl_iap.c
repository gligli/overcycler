...
void find_erase_prepare_sector(unsigned cclk, unsigned flash_address_)
{
    unsigned i;
    ...
    {
        if(flash_address_ < SECTOR_END(i))
        {
            if( flash_address_ == SECTOR_START(i))
            {
...
void write_data(unsigned cclk,unsigned flash_address_,unsigned * flash_data_buf, unsigned count)
{
	__disable_irq();
    param_table[0] = COPY_RAM_TO_FLASH;
    param_table[1] = flash_address_;
