...
__attribute__ ((used)) void UART2_IRQHandler(void)
{
	...
	synth_uartEvent(UART_ReceiveByte(UART_2));
}

void uartMidi_init(void)
{
//oc3	CLKPWR_SetPCLKDiv(CLKPWR_PCLKSEL_UART2,CLKPWR_PCLKSEL_CCLK_DIV_1);	
//oc3	PINSEL_SetPinFunc(0,11,1);
	
	UART_CFG_Type uart;
	UART_ConfigStructInit(&uart);
	uart.Baud_rate=31250;
	UART_Init(UART_2,&uart);
